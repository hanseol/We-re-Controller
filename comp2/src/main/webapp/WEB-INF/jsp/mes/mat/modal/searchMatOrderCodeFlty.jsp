	...
	<div class="panel panel-headline">
		<div class="panel-heading">
			<div class="panel-body">
				...
				<div id="matOrderFltyModalGrid"></div>
			</div>
		</div>
	</div>
</div>
...
$(document).ready(function() {
	...
	$(document).on("click","button[id=ok]", function(){
		...
		var chkRowKeys = matOrderFltyModalGrid.getCheckedRowKeys();
		for(var i=0; i<chkRowKeys.length; i++){
			erpMaterialOrderCode = matOrderFltyModalGrid.getValue(chkRowKeys[i],'erpMaterialOrderCode');
			erpMaterialRequestDate = matOrderFltyModalGrid.getValue(chkRowKeys[i],'erpMaterialRequestDate');
			erpMaterialOrderQty = matOrderFltyModalGrid.getValue(chkRowKeys[i],'erpMaterialOrderQty');
			erpMaterialUnitPrice = matOrderFltyModalGrid.getValue(chkRowKeys[i],'erpMaterialUnitPrice');
			erpVendorCode = matOrderFltyModalGrid.getValue(chkRowKeys[i],'erpVendorCode');
			comCodeDetailName = matOrderFltyModalGrid.getValue(chkRowKeys[i],'comCodeDetailName');
			comMaterialCode = matOrderFltyModalGrid.getValue(chkRowKeys[i],'comMaterialCode');
			comMaterialName = matOrderFltyModalGrid.getValue(chkRowKeys[i],'comMaterialName');
			comMaterialSize = matOrderFltyModalGrid.getValue(chkRowKeys[i],'comMaterialSize');
			comMaterialUnit = matOrderFltyModalGrid.getValue(chkRowKeys[i],'comMaterialUnit');
	...
	$(document).on("click", "button[id=modalSearch]", function() {
		...
		matOrderFltyModalGrid.readData(1, readParams, true);
	});
	
	const dataSource = {
		api : {
			readData : {
				url : '${pageContext.request.contextPath}/ajax/searchMatOrderFlty',
				method : 'GET'
			}
		},
	...
	matOrderFltyModalGrid = new tui.Grid({
		el : document.getElementById('matOrderFltyModalGrid'),
		rowHeaders : [ 'checkbox' ],
		data : dataSource,
			scrollX: true,
	        scrollY: true,
